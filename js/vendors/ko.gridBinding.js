...
(function ($) {
    ...
    $.fn.tabletogrid = function (settings) {
        settings = settings || {};
        $(this).each(function () {
            if (this.jqGrid) { return; }
            var element = $(this).width('99%'),
                options = settings.opts || { datatype: 'local', colModel: [], colNames: [], height: '100%', altRows: true, viewrecords: true },
                pagerOptions = settings.pager || { target: '#pager', rowNum: 10, rowList: [10, 20, 50] },
                navOptions = settings.nav || {refresh:false,edit:false,add:false,del:false,view:false,search:false}
            ...
            if (!settings.options || !settings.options.colModel || settings.options.colModel.length < 1) {
	            buildColModel(element, options);
            }
            element.empty().jqGrid(options);
            if (options.pager) {
				element.jqGrid('navGrid',options.pager,navOptions);
            }
        });
    };
